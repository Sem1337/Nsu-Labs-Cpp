...
Ship Parser::parseShip(std::string ship)
{
	...
	firstSide = { tokenized[0][0] - 'a',atoi(tokenized[1].c_str()) };
	secondSide = { tokenized[2][0] - 'a', atoi(tokenized[3].c_str()) };
	if (firstSide.first < 1 || firstSide.first > 10
		|| firstSide.second < 1 || firstSide.second > 10
		|| secondSide.first < 1 || secondSide.first > 10
		|| secondSide.second < 1 || secondSide.second > 10)throw std::exception("incorrect ship format");
